...
function Product() {
    ...
    const handleProduct = async () => {
        try {
            const url = 'http://localhost:8080/api/get-product';
    ...
    const handleDelete = async (id) => {
        try {
            const url = `http://localhost:8080/api/delete-product/${id}`
    ...
    const varientExist = async (productId, navigate) => {
        try {
            const url = `http://localhost:8080/api/get-variant/${productId}`;
            const response = await axios.get(url);
