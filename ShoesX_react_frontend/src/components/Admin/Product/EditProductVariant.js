...
//   const [variantSections, setVariantSections] = useState(state.variants);
...
//   //   const imageToRemove = variant.imagesPreviews.find(
//   //     (img) => img.id === imageId
//   //   );
...
//   //     } else {
//   //       // If it's a new image, remove from newImageFiles array
...
//       filename: file.name, // Temporary filename
...
//       // Prepare variants without images (we’ll attach images separately)
//       const variantsData = variantSections.map((variant, index) => {
//         const { newImageFiles, ...rest } = variant; // exclude newImageFiles for now
//         return {
//           ...rest,
//           index
//         };
...
//       // Append images under correct field names expected by backend: variants[${index}].images
...
//       const url = `http://localhost:8080/api/edit-variant/${state._id}`;
...
//         toast.success(response.data.message); // Use response.data.message here
//         setTimeout(() => {
...
//                 value={section.color ? variantOptions.find(option => option.value === section.color._id) : null}
...
//                 components={animatedComponents}
//                 options={sizeOptions}
//                 value={section.size ? sizeOptions.find(option => option.value === section.size._id) : null}
...
//                       <img
//                         src={img.isExisting ? `http://localhost:8080/${img.filename.replace(/\\/g, "/")}` : img.previewUrl}
//                         alt="preview"
    ...
    if (state.variants && sizeOptions.length > 0 && variantOptions.length > 0) {
      const initializedVariants = state.variants.map((variant) => ({
        ...
        imagesPreviews: variant.images ? variant.images.map((img, index) => ({
          id: `existing-${index}`,
          filename: img,
          previewUrl: `http://localhost:8080/${img.replace(/\\/g, "/")}`,
          isExisting: true
        })) : [],
        removedImages: []
      }));
